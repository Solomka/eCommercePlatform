...
public final class TypeVOValidator implements Validator {
	...
	public void validate(Object o, Errors errors) {
		 ...
		 */
		/*
		 * if (type.getTypeId() < 0) { errors.rejectValue("typeId",
		 * "error.type.vo.typeId", String.format("typeId '%d' is invalid",
		 * type.getTypeId())); }
		 */
		if (TextUtils.nullOrEmpty(type.getTypeName()) || !TYPE_NAME_PATTERN.matcher(type.getTypeName()).matches()) {
