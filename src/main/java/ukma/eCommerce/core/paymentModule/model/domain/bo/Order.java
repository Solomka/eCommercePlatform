...
import static ukma.eCommerce.core.paymentModule.model.domain.vo.types.OrderStatus.RETURNED;
...
public final class Order {
	 ...
	 * <td>CANCELED</td>
	 * <td>RETURNED</td>
	 * <td>FULFILLED PARTIALLY</td>
	 * <td>ISSUED PARTIALLY</td>
	 * <td>FULFILLED</td>
	 * <td>ISSUED</td>
	 * </tr>
	 * <tr>
	 * <td>IN_SENDER_WAREHOUSE</td>
	 * <td>yes</td>
	 * <td>yes</td>
	 * <td>yes</td>
	 * <td>yes</td>
	 ...
	 *            fulfilment date to set
	 * @param orderStatus
	 *            order status to set
	 * @param shipmentStatus
	 *            shipmentStatus to set
	 * @throws IllegalArgumentException
	 *             if one of parameters is invalid
	 */
	...
	/**
	 * Replaces current shipment with new new one. Allowed transition matrix
	 * looks like <br>
	 * <table>
	 * <tr>
	 * <td>Shipment\Order</td>
	...
	private OrderStatus checkOrderStatus(final DateTime fulfilmentDate, final OrderStatus status) {
		...
		if (fulfilmentDate != null && (status == CREATED || status == CANCELED || status == PAID))
			throw new IllegalArgumentException(
					"order cannot have status created/canceled or paid at the same time when fulfilled");
	...
	/**
	 * Checks order status to be set. Allowed transitions are CREATED -> PAID ->
	 * FULFILLED/PARTIALLY -> ISSUED/PARTIALLY. If status is CREATED, then new
	 * status can be CANCELED, in other cases - RETURNED
	 *
	 * @param status
	 *            new status to check
	 */
	...
	/**
	 * Checks shipment status to be set. Allowed transitions are
	 * IN_SENDER_WAREHOUSE -> IN_DELIVERY -> DELIVERED/PARTIALLY
	 *
	 * @param status
	 *            new status to check
	 */
	...
	private void checkState(ShipmentStatus shipmentStatus, OrderStatus orderStatus) {
		...
		if (shipmentStatus == ShipmentStatus.DELIVERED_PARTIALLY && orderStatus != OrderStatus.FULFILLED_PARTIALLY
			...
			throw new IllegalStateException(
					"while DELIVERED_PARTIALLY should have status FULFILLED_PARTIALLY/ISSUED_PARTIALLY");
		...
		if (shipmentStatus == ShipmentStatus.DELIVERED && orderStatus != RETURNED && orderStatus != ISSUED_PARTIALLY
			...
			throw new IllegalStateException(
					"while DELIVERED should have status RETURNED/ISSUED_PARTIALLY/FULFILLED/ISSUED");
	}
