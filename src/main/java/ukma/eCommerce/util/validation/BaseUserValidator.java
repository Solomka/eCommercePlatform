...
class BaseUserValidator<T extends User> implements IValidator<T> {
	...
	protected static final Pattern PLAIN_STR_PATTERN = Pattern
			.compile(String.format("[a-zA-z]{%d,%d}", minStrLen, maxStrLen));// what
																				// about
																				// allowed
																				// string
																				// length?
	...
	public ValidationResult validate(T user) {
		...
		if (id == null || id.length() < minIdLen || id.length() > maxIdLen) {
			return ValidationResult.createInvalidResult(String.format(
					"User id '%s' is invalid, it should contain %d-%d characters", user.getId(), minIdLen, maxIdLen));
		}
