...
final class SellerValidator extends BaseUserValidator<Seller> {
	...
	public ValidationResult validate(Seller seller) {
		...
		if(!TextUtils.nullOrEmpty(seller.getCompany()) && 
				!PLAIN_STR_PATTERN.matcher(seller.getCompany()).matches()) {
			return ValidationResult.createInvalidResult(String.format("Company name '%s' is invalid", seller.getCompany()));
		}
